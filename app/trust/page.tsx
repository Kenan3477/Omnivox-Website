      ...
      <Section>
        <SectionContainer>
          ...
          <p className="mt-4 text-slate-600 leading-relaxed max-w-3xl">
            This page is the public summary, not a substitute for the DPA. We do not list SOC 2, ISO 27001, PCI DSS,
            or SSO as current certifications because we have not attested those. Recording consent, TPS/CTPS, and
            lawful basis for your lists stay with you as controller. We provide org isolation, DNC, audit logs,
            recording toggles, role-based access, and a DPA at onboarding so your team can operate accountably. Email{" "}
            ...
            if you need the DPA before a pilot.
              ...
              <div className="mt-8 grid md:grid-cols-2 gap-6">
                  ...
                  <ul className="mt-4 space-y-3">
                    {roles.controller.points.map((p) => (
                      <li key={p} className="flex gap-2 text-sm text-slate-700">
                        <CheckIcon className="h-4 w-4 text-blue-500 shrink-0 mt-0.5" />
                        {p}
                      </li>
                    ))}
                  </ul>
                ...
                <div className="rounded-xl border border-indigo-200 bg-white p-6">
                  <p className="text-sm font-bold text-indigo-800">{roles.processor.label}</p>
                  <p className="text-xs uppercase tracking-wider text-slate-400 mt-1">{roles.processor.subtitle}</p>
                  <ul className="mt-4 space-y-3">
                    {roles.processor.points.map((p) => (
                      <li key={p} className="flex gap-2 text-sm text-slate-700">
                        <CheckIcon className="h-4 w-4 text-indigo-500 shrink-0 mt-0.5" />
                        {p}
                      </li>
                    ))}
                  </ul>
          ...
          <div>
            <div className="mt-12 rounded-2xl border border-blue-200 bg-gradient-to-br from-blue-50 to-white p-8">
              ...
              <p className="mt-2 text-slate-600 leading-relaxed max-w-2xl">
                ...
                <a href={`mailto:${siteConfig.contactEmail}`} className="text-blue-700 font-medium hover:text-blue-600">
                  {siteConfig.contactEmail}
                </a>
                . Read our{" "}
                <Link href="/privacy" className="text-blue-700 font-medium hover:text-blue-600">
                  Privacy Policy
                </Link>{" "}
