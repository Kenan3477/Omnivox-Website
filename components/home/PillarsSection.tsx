      ...
      <SectionContainer>
        ...
        <div className="grid md:grid-cols-3 gap-4">
          {pillars.map((pillar) => (
            ...
            >
              <p className="text-[10px] font-bold uppercase tracking-widest text-blue-600">{pillar.eyebrow}</p>
              ...
              <ul className="mt-4 space-y-1.5 border-t border-slate-100 pt-4">
                {pillar.bullets.slice(0, 3).map((b) => (
                  <li key={b} className="flex items-start gap-2 text-xs text-slate-700">
                    <CheckIcon className="h-3 w-3 text-blue-500 mt-0.5 shrink-0" />
                    {b}
                  </li>
                ))}
              </ul>
              <Link href={pillar.href} className="mt-4 text-xs font-semibold text-blue-600 hover:text-blue-500">
                {pillar.cta} →
              </Link>
            </article>
          ))}
