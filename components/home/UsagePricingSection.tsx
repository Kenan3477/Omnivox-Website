        ...
        <div className="mt-4 grid md:grid-cols-2 gap-4">
          {bundledPlanOptions.map((plan) => (
            <div key={plan.name} className="rounded-lg border border-slate-100 bg-slate-50 p-4">
              ...
              <p className="text-[10px] uppercase tracking-wider text-blue-700 mt-1">{plan.bestFor}</p>
              <p className="mt-2 text-xs text-slate-600 leading-relaxed">{plan.detail}</p>
            </div>
          ))}
        </div>
        {variant === "section" && (
          <Link href="/pricing#calculator" className="mt-4 inline-flex text-xs font-semibold text-blue-600 hover:text-blue-500">
