        ...
        <div className="grid lg:grid-cols-2 gap-6">
          ...
          <div className="rounded-lg border border-slate-200 bg-slate-50 p-5 md:p-6">
            ...
            <div className="mt-5 grid sm:grid-cols-2 gap-4">
              <div className="rounded-lg border border-slate-200 bg-white p-4">
                <p className="text-xs font-bold text-blue-700">{roles.controller.label}</p>
                ...
                <ul className="mt-3 space-y-2">
                  {roles.controller.points.map((p) => (
                    <li key={p} className="flex gap-2 text-xs text-slate-700">
                      <CheckIcon className="h-3 w-3 text-blue-500 shrink-0 mt-0.5" />
                      {p}
                    </li>
                  ))}
                </ul>
              ...
              <div className="rounded-lg border border-slate-200 bg-white p-4">
                <p className="text-xs font-bold text-indigo-700">{roles.processor.label}</p>
                ...
                <ul className="mt-3 space-y-2">
                  {roles.processor.points.map((p) => (
                    <li key={p} className="flex gap-2 text-xs text-slate-700">
                      <CheckIcon className="h-3 w-3 text-indigo-500 shrink-0 mt-0.5" />
                      {p}
                    </li>
                  ))}
                </ul>
          ...
          <div className="rounded-lg border border-slate-200 bg-white p-5 md:p-6">
            ...
            <div className="mt-5 flex flex-wrap gap-4 text-xs font-semibold">
              <Link href="/trust" className="text-blue-600 hover:text-blue-500">
                Full trust &amp; security →
              </Link>
