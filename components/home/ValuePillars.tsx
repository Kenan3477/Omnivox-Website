...
const pillars = [
  {
    title: "Top up",
    description: "Buy prepaid credits in £50, £100, £250, or £500 packs. One wallet for every call.",
    icon: (
  ...
  {
    title: "Dial",
    description: "Manual and preview outbound from the browser. Review each contact, skip or call — pilot-proven.",
    icon: (
      <svg className="h-7 w-7" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
  ...
  {
    title: "Track usage",
    description: "~5p per minute, inbound or outbound. Low-balance warnings on Work. Top up in Admin → Call Credits.",
    icon: (
      <svg className="h-7 w-7" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
          ...
          {pillars.map((pillar, i) => (
            <AnimateOnScroll key={pillar.title} delay={i * 100}>
              <div className="glass-card-light p-8 h-full">
                <div className="mb-4 flex h-14 w-14 items-center justify-center rounded-xl bg-cyan-50 text-cyan-600">
                  {pillar.icon}
                </div>
