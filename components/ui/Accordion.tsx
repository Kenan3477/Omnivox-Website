          ...
          >
            ...
            <span
              className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-full transition-transform duration-200 group-open:rotate-45 ${
                light ? "bg-slate-100 text-slate-600" : "bg-white/10 text-blue-400"
              }`}
              aria-hidden="true"
            >
...
export function SectionHeading({
  ...
  return (
    <div className={`mb-12 md:mb-16 ${align === "center" ? "text-center mx-auto max-w-3xl" : "max-w-2xl"}`}>
      {eyebrow && (
        <p className={`mb-3 text-sm font-semibold uppercase tracking-wider ${dark ? "text-blue-400" : light ? "text-blue-600" : "text-blue-400"}`}>
          {eyebrow}
        </p>
      )}
      <Heading
...
export function Card({ children, className = "", hover = false }: CardProps) {
  return (
    <div
      className={`rounded-2xl border border-slate-200 bg-white p-6 md:p-8 ${hover ? "transition-all duration-300 hover:border-blue-300 hover:shadow-lg hover:-translate-y-1" : ""} ${className}`}
    >
      {children}
    </div>
  );
