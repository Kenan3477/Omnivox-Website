    ...
    <div className="grid min-h-[300px] grid-cols-12 bg-slate-950 text-slate-200 text-sm">
      ...
      <div className="col-span-3 border-r border-white/5 bg-slate-900/80 p-4 hidden sm:block">
        <div className="flex items-center gap-2 mb-6">
          <div className="h-8 w-8 rounded-lg bg-gradient-to-br from-indigo-600 to-blue-500 flex items-center justify-center text-white font-bold text-sm">O</div>
        ...
        <nav className="space-y-1">
          {["Queue", "My contacts", "History"].map((item, i) => (
            <div
              key={item}
              className={`rounded-lg px-3 py-2 text-xs ${i === 0 ? "bg-blue-500/15 text-blue-300 font-medium" : "text-slate-500"}`}
            >
              {item}
            </div>
          ))}
      ...
      <div className="col-span-12 sm:col-span-3 border-r border-white/5 p-4 bg-slate-900/40">
        ...
        <div className="space-y-2">
          {[
            ...
            <div
              key={c.name}
              className={`rounded-xl p-3 border ${c.active ? "border-blue-500/40 bg-blue-500/10" : "border-white/5 bg-white/[0.02]"}`}
            >
      ...
      <div className="col-span-12 sm:col-span-6 p-5 md:p-6">
        <div className="flex items-start justify-between gap-4 mb-5">
          <div>
            ...
            <p className="text-sm text-blue-400/80 mt-1">+44 7700 900123</p>
          </div>
        ...
        <div className="flex gap-2 mb-5">
          <button type="button" className="flex-1 rounded-xl bg-gradient-to-r from-blue-500 to-blue-400 py-3 text-sm font-bold text-slate-900 shadow-glow">
            Dial
          </button>
  ...
  return (
    <div className="p-5 md:p-6 bg-slate-50 min-h-[240px]">
      <div className="flex items-center justify-between mb-6">
        ...
        <button type="button" className="rounded-xl bg-blue-500 px-4 py-2 text-sm font-semibold text-white">Top up</button>
      </div>
